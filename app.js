...
function activateTab(key){
  // highlight all tabs (top and bottom) with same key
  document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));
  document.querySelectorAll(`.tab[data-tab="${key}"]`).forEach(b=>b.classList.add("active"));
  // show correct panel
...
(function bindSaveDraft(){
  ...
  btn.addEventListener("click", () => {
    ...
    if(state.draft){
      ...
      if(status) status.textContent = "Guardado: " + new Date().toLocaleString();
    }
